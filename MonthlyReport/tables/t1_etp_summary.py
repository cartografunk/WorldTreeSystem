...
def _agg_by_type(df: pd.DataFrame, alloc_type: str) -> pd.DataFrame:
    ...
    NUEVO: Agrega conteo de survival_metric_source y planting_year
    """
    ...
    # NUEVO: Conteo de survival_metric_source (sobre contratos activos sin OOP)
    df_active = _subset_for_survival(df)
    if "survival_metric_source" in df_active.columns:
        # Crear conteo por cada source
        metric_source_counts = (
            df_active.groupby(["region", "etp_year_raw", "survival_metric_source"], dropna=False)["contract_code"]
            .nunique()
            .reset_index()
        )
        # Convertir a formato "source: count"
        metric_source_summary = (
            metric_source_counts.groupby(["region", "etp_year_raw"])
            .apply(lambda g: {row["survival_metric_source"]: row["contract_code"]
                              for _, row in g.iterrows()
                              if pd.notna(row["survival_metric_source"])})
            .reset_index(name="Survival Metric Source Count")
        )
    else:
        metric_source_summary = pd.DataFrame(columns=["region", "etp_year_raw", "Survival Metric Source Count"])

    # NUEVO: Conteo de planting_year (sobre df_no_omit, mismo subset que Inventory Counts)
    if "planting_year" in df_no_omit.columns:
        # Crear conteo por cada planting year
        planting_year_counts = (
            df_no_omit.groupby(["region", "etp_year_raw", "planting_year"], dropna=False)["contract_code"]
            .nunique()
            .reset_index()
        )
        # Convertir a formato {year: count}
        planting_year_summary = (
            planting_year_counts.groupby(["region", "etp_year_raw"])
            .apply(lambda g: {int(row["planting_year"]) if pd.notna(row["planting_year"]) else None: row["contract_code"]
                              for _, row in g.iterrows()})
            .reset_index(name="Planting Year Counts")
        )
    else:
        planting_year_summary = pd.DataFrame(columns=["region", "etp_year_raw", "Planting Year Counts"])
    ...
    sums_totals = (
        ...
        df_no_omit.groupby(["region", "etp_year_raw"], dropna=False).size().reset_index(name="__dummy__").drop(
            columns="__dummy__")
    )
    ...
    sums_survival = (
        ...
        df_active.groupby(["region", "etp_year_raw"], dropna=False).size().reset_index(name="__dummy__").drop(
            columns="__dummy__")
    )
    ...
    out = out.merge(metric_source_summary, on=["region", "etp_year_raw"], how="left")  # Inventory Counts
    out = out.merge(planting_year_summary, on=["region", "etp_year_raw"], how="left")  # NUEVO: Planting Year Counts
    out = out.merge(sums_totals, on=["region", "etp_year_raw"], how="left")
    out = out.merge(sums_survival, on=["region", "etp_year_raw"], how="left")
...
def build_etp_summary(mbt: pd.DataFrame) -> pd.DataFrame:
    ...
    if not frames:
        return pd.DataFrame(columns=["Allocation Type", "Region", "ETP Year", "Total Contracts",
                                     "Contract Codes", "Survival Metric Source Count",
                                     "Planting Year Counts", "Survival"])
    ...
    # Quitar solo la columna de depuración Contract Codes (NO las de counts)
    out = out.drop(columns=["Contract Codes"], errors="ignore")

    # Asegurar que las columnas de counts existen
    if "Survival Metric Source Count" not in out.columns:
        out["Survival Metric Source Count"] = None
    if "Planting Year Counts" not in out.columns:
        out["Planting Year Counts"] = None

    # Columnas fijas
    fixed_left_base = ["Allocation Type", "Region", "ETP Year", "Total Contracts",
                       "Survival Metric Source Count", "Planting Year Counts"]
    fixed_left = [c for c in fixed_left_base if c in out.columns]

    fixed_right = [c for c in ["Survival %"] if c in out.columns]

    # El resto son columnas de status
